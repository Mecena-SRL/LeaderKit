# LeaderKit 0.1 — script Python (archivio)

Prima versione di LeaderKit, uno script da Workspace › Scripts. È superata dal
generatore nella libreria Effetti (vedi il README principale) ma resta nel repo
come riferimento del motore di calcolo (`src/leaderkit/`) ed è ancora costruita
da `tools/build.py`.
...
## Da verificare in Resolve (0.1)

1. **ImportFusionComp** su un clip di sequenza PNG: la composizione generata
   (formato `.comp`) viene accettata e renderizzata.
2. **Resa grafica** su 16:9, 1.85 DCI e 2.39; font "Open Sans".
3. **Braccio rotante** con `comp.RenderStart`.
4. **Marker**: `AddMarker` usa frameId relativi all'inizio timeline.
5. **Timeline annidata**: `Timeline.GetMediaPoolItem()`.
6. **UIManager** nella versione free di Resolve.
